% close all;
% clear all;
function failuremodel_mod2(n1, n2,topology,inter,avg_deg)


% n1=500;  %number of nodes in network X
% n2=500;  %number of nodes in network Y
p1=(avg_deg/(n1-1)); %the probability of having a link between nodes of network X
p2=(avg_deg/(n2-1)); %the probability of having a link between nodes of network Y
...
% m1  = 2*mult; %m11: number of nodes a new added node is connected to, 1 <= m1 < m10
% m20 = 3; %m20: number of initially placed nodes of network Y
% m2  = 2*mult; %m21: number of nodes a new added node is connected to, 1 <= m2 < m20

% variables for Watts Strogatz Model/Small Workd Network
deg1 = avg_deg; %d1: mean degree of network X, deg%2=0 && 0<deg<n-1
rp1 = 0.5; %r1: rewiring probability of network X, 0<=rp<=1
deg2 = avg_deg; %d2: mean degree of network Y, deg%2=0 && 0<deg<n-1 
...
%     avr(i)=NF2;
% end
avg_deg = num2str(avg_deg);



temp = strcat("scenari-11&12n=100",inter,"-",topology,"k=10%-avgdeg=",avg_deg,".mat") ;
...
temp = strcat("scenari-21&22n=100",inter,"-",topology,"k=10%-avgdes=",avg_deg,".mat") ;
