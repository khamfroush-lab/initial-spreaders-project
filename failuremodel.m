close all;
clear all;
n1=100;  %number of nodes in network X
n2=100;  %number of nodes in network Y
p1=0.04; %the probability of having a link between nodes of network X
p2=0.04; %the probability of having a link between nodes of network Y
...
%         d1=rand(1,1);
%         if d1<=p12
%             adj_xy(e1,y)=1;  %creating the adj matrix for interdependency between Y and X(Y is parent of X)
%         end
%     end
% end
% 
...
% r=unique(r2);
% 
% r_X=r(1,1:8);                     %Samuel are you sure to get 8 data point ?
% for e=1:length(r_X)
%     e1=r_X(e);
%     for y=1:n2
%         d1=rand(1,1);
%         if d1<=p12            % samuel- x is parent of y ?
%             adj_xy(e1,y)=1;  %creating the adj matrix for interdependency between Y and X(Y is parent of X)  
%         end
%     end
% end
...
%         d1=rand(1,1);
%         if d1<=p12
%             adj_xy(e,y)=1;  %creating the adj matrix for interdependency between X and Y(X is parent of Y)
%         end
%     end
% end
% 
...
%         d1=rand(1,1);
%         if d1<=p21
%             adj_yx(e,y)=1;  %creating the adj matrix for interdependency between Y and X(Y is parent of X)
%         end
%     end
% end
%*****************************
...
adj_xy=zeros(n1,n2);
adj_yx=zeros(n2,n1);
max_degree_x=node_degree_order_x(1,1:10);
max_degree_y=node_degree_order_y(1,1:10);
...
F=ceil(n1*rand(1,30)); 
F=unique(F);
F1=F(1,1:6);     % initial failures in network A
%--------
a = n1;
b = n1+n2;
...
F=unique(F);
F_a=F(1,1:3);
F2= [ F_a F_b ];   % initial failures in network A and B
...
save scenari-11&12dense&designedSF-ER.mat vector_n_fail_time vector_failed_low_betx vector_failed_low_bety vector_failed_low_degx vector_failed_low_degy vector_failed_high_betx vector_failed_high_bety vector_failed_high_degx vector_failed_high_degy vector_Dgr1_11 vector_Dgr1_12 vector_F1 vector_F2 vector_n_fail_time2 vector_failed_low_betx2 vector_failed_low_bety2 vector_failed_low_degx2 vector_failed_low_degy2 vector_failed_high_betx2 vector_failed_high_bety2 vector_failed_high_degx2 vector_failed_high_degy2 vector_Dgr1_21 vector_Dgr1_22 vector_S_time1 vector_S_time2 
